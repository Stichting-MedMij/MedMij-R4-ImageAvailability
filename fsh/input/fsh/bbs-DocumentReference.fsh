...
* type from http://decor.nictiz.nl/fhir/ValueSet/2.16.840.1.113883.2.4.3.11.60.133.11.1--20230808113539 (required)
  ...
  * ^definition = """
    ...
    """
  * ^comment = """
    The value set bound to this element is a thesaurus managed, owned and distributed by DHD (namely the Verrichtingenthesaurus (VT)). Every concept in the VT is linked to a unique SNOMED concept, but it is no SNOMED refset, and in particular it is not published on the SNOMED browsers. In order to access the contents of the VT, a DHD license is required.
